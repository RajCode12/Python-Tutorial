...
df1 = pd.read_csv('Marathon/sales_data_2022.csv')
print(df1)
print('*******************************************')

df2 = pd.read_csv('Marathon/sales_data_2023.csv')
print(df2)
print('*******************************************')
...
total_sales_by_product = merge_df.groupby('Product')['Sales'].sum().reset_index()
print('Total Sales by Product : ')
print(total_sales_by_product)
print('*******************************************')

# region with highest total sales
highest_sales_region = merge_df.groupby('Region')['Sales'].sum().idxmax()
print('Top Region : ', highest_sales_region)
print('*******************************************')

# product with highest total sales
highest_sales_product = merge_df.groupby('Product')['Sales'].sum().idxmax()
